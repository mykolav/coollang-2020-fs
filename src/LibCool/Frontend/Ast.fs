...
type Node private () =
    ...
    static member Mk<'TValue>(first: uint32, last: uint32, value: 'TValue) =
        Node.Mk(span = { First = first; Last = last }, value = value)
           
type Ast = { Program: Node<Program> }

type ID = ID of value: string
    with member this.Value = let (ID value) = this in value end 
    
type TYPE_NAME = TYPE_NAME of value: string
    with member this.Value = let (TYPE_NAME value) = this in value end 

type INT = INT of value: int
    with member this.Value = let (INT value) = this in value end 

type STRING = STRING of value: string
    with member this.Value = let (STRING value) = this in value end 

type BOOL = True | False

type Program = { ClassDecls: Node<ClassDecl>[] }
...
type Extends = Info of ExtendsInfo
             | Native
type ExtendsInfo = { TYPE_NAME: Node<TYPE_NAME>
                     Actuals: Node<Expr>[] }

type VarFormal = { ID: Node<ID>; TYPE_NAME: Node<TYPE_NAME> }

[<RequireQualifiedAccess>]
type Feature = Method of MethodInfo
             | Attr of AttrInfo
             | Block of Block

type MethodInfo = { Override: bool
                    ID: Node<ID>
                    Formals: Node<Formal>[]
                    TYPE_NAME: Node<TYPE_NAME>
                    MethodBody: Node<MethodBody> }
[<RequireQualifiedAccess>]
type MethodBody = Expr of Expr
                | Native

type Formal = { ID: Node<ID>; TYPE_NAME: Node<TYPE_NAME> }

[<RequireQualifiedAccess>]
type AttrInfo = { ID: Node<ID>
                  TYPE_NAME: Node<TYPE_NAME>
                  AttrBody: Node<AttrBody> }

[<RequireQualifiedAccess>]
type AttrBody = Expr of Expr
              | Native

[<RequireQualifiedAccess>]
type Block = Info of BlockInfo
           | Braced of BlockInfo option
type BlockInfo = { Stmts: Node<Stmt>[]; Expr: Node<Expr> }

[<RequireQualifiedAccess>]
type Stmt = VarDecl of VarDeclInfo
          | Expr of Expr
type VarDeclInfo = { ID: Node<ID>; TYPE_NAME: Node<TYPE_NAME>; Expr: Node<Expr> }

[<RequireQualifiedAccess>]
type Expr = Assign of left:Node<ID> * right:Node<Expr>
          ...
          | Match of expr:Node<Expr> * cases:Cases
...
type Cases = Node<Case> * Node<Case>[]
