...
type Lexer(_source: Source, _diags: DiagnosticBag) =
    ...
    let try_punctuator (ch: char): Token option =
        let token_kind_opt =
            match ch with
            | '+' -> Some TokenKind.Plus
    ...
    let lex_next (): Token =
        ...
        if is_letter ch
        then
            let sb_id = StringBuilder(ch.ToString())
        ...
        if is_digit ch
        then
            let sb_int = StringBuilder(ch.ToString())
            let mutable token_complete = false
        ...
        then
            ...
            while Option.isNone token_opt do
                ...
                if line_break_span <> HalfOpenRange.Invalid
                then
                    _diags.Add(Diagnostic.Of(DiagnosticSeverity.Error,
