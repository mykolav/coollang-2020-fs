...
type AstWalker private (_listener: AstListener) as this =
    ...
    let walk_block_info (block_info: BlockInfo, key: Guid, span: HalfOpenRange): unit =
        ...
        _listener.EnterBlockLastExpr(block_expr.Value, block_expr.Key, block_expr.Span)
        this.WalkExpr(block_expr.Value, block_expr.Key, block_expr.Span)
        _listener.LeaveBlockLastExpr(block_expr.Value, block_expr.Key, block_expr.Span)
    ...
    let walk_match_case_pattern (pattern: Pattern, key: Guid, span: HalfOpenRange): unit =
        ...
        _listener.LeaveMatchCasePattern(pattern, key, span)
    ...
    let walk_match (expr: Node<Expr>, cases_hd: Node<Case>, cases_tl: Node<Case> [], key: Guid, span: HalfOpenRange): unit =
        ...
        _listener.EnterMatchExpr(expr.Value, expr.Key, expr.Span)
        this.WalkExpr(expr.Value, expr.Key, expr.Span)
        _listener.LeaveMatchExpr(expr.Value, expr.Key, expr.Span)

        _listener.VisitMATCH()
    ...
    let walk_implicit_this_dispatch (method_id: Node<ID>, actuals: Node<Expr> [], key: Guid, span: HalfOpenRange): unit =
        ...
        _listener.VisitID(method_id.Value, method_id.Key, method_id.Span)
        walk_actuals (actuals)
        _listener.LeaveImplicitThisDispatch(method_id, actuals, key, span)
    ...
    let walk_super_dispatch (method_id: Node<ID>, actuals: Node<Expr> [], key: Guid, span: HalfOpenRange): unit =
        ...
        _listener.VisitID(method_id.Value, method_id.Key, method_id.Span)
        walk_actuals (actuals)
        _listener.LeaveSuperDispatch(method_id, actuals, key, span)
    ...
    let walk_object_creation (type_name: Node<TYPE_NAME>, actuals: Node<Expr> [], key: Guid, span: HalfOpenRange): unit =
        ...
        walk_actuals (actuals)
        _listener.LeaveObjectCreation(type_name, actuals, key, span)
    ...
    let walk_method (method_info: MethodInfo, key: Guid, span: HalfOpenRange): unit =
        ...
        _listener.EnterMethodBody(node_body.Value, node_body.Key, node_body.Span)
        ...
        _listener.LeaveMethodBody(node_body.Value, node_body.Key, node_body.Span)
    ...
    let walk_attr (attr_info: AttrInfo, key: Guid, span: HalfOpenRange): unit =
        ...
        _listener.EnterAttrBody(node_body.Value, node_body.Key, node_body.Span)
        ...
        _listener.LeaveAttrBody(node_body.Value, node_body.Key, node_body.Span)
    ...
    member private __.WalkExpr(expr: Expr, key: Guid, span: HalfOpenRange): unit =
        ...
        | Expr.Id value ->
            _listener.VisitId(value, key, span)
        | Expr.Int value ->
            _listener.VisitInt(value, key, span)
        | Expr.Str value ->
            _listener.VisitStr(value, key, span)
        | Expr.Bool value ->
            _listener.VisitBool(value, key, span)
        | Expr.This ->
            _listener.VisitThis(key, span)
        | Expr.Null ->
            _listener.VisitNull(key, span)
        | Expr.Unit ->
            _listener.VisitUnit(key, span)

    member private __.Walk(ast: Ast): unit = walk_ast (ast)
